...
class PHP {
  ...
  static async loadPhp(version = "7.4.31") {
    //const { ccall } = await import(`./php-web/builded_app/demo_jolitypo/php-web.mjs`).then(module => module.default({
    const { ccall } = await import(`./php-web/builded_app/php-web.mjs`).then(module => module.default({
      print(data) {
        if (!data) {
          return;
        }
    ...
    PHP.runPhp = (code) => ccall("phpw", null, ["string"], ["/app/src/index.php"]);
    return PHP.runPhp;
  }
}
